(() => {
  ...
  const loadPrivateItems = async () => {
    ...
    try {
      ...
      result.items.forEach((item)=>{const li=document.createElement('li');const jump=document.createElement('button');jump.type='button';jump.textContent=`${item.item_type} · page ${item.page_number}`;jump.addEventListener('click',()=>setPage(item.page_number));li.appendChild(jump);if(item.note_text){const span=document.createElement('span');span.textContent=` — ${item.note_text}`;li.appendChild(span);}const del=document.createElement('button');del.type='button';del.textContent='Delete';del.addEventListener('click',async()=>{await api(`reading/items/${item.id}`,{method:'DELETE',body:{expected_version:item.version},headers:{'Idempotency-Key':crypto.randomUUID?.()||`delete-${item.id}-${Date.now()}-${Math.random()}`}});loadPrivateItems();});li.appendChild(del);ul.appendChild(li);});section.appendChild(ul);privateItems.replaceChildren(section);
