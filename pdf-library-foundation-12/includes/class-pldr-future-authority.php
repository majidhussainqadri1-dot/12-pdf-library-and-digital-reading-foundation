...
final class PLDR_Future_Authority {
    public static function lookup(string $type, string $value, bool $force = false) {
        global $wpdb;
        $type = sanitize_key($type); $value = trim(sanitize_text_field($value));
        ...
        $provider = sanitize_text_field((string) ($result['provider'] ?? 'adapter'));
        ...
        $wpdb->replace(PLDR_Core::table('authority_cache'), array('identifier_type'=>$type,'identifier_value'=>$value,'provider'=>$provider,'result_json'=>$encoded,'provenance_json'=>wp_json_encode($provenance),'expires_at'=>gmdate('Y-m-d H:i:s', time()+30*DAY_IN_SECONDS),'created_at'=>PLDR_Core::now(),'updated_at'=>PLDR_Core::now()));
