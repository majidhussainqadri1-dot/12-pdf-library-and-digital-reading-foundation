...
final class PLDR_Future_Preservation {
    public static function assess(int $edition_id,bool $verify=false):array {
        ...
        $integrity='unknown';$error='';$findings=array();
        if($verify){
            if(is_wp_error($path)){
        ...
        if(is_array($external)){
            ...
            foreach((array)($external['findings']??array()) as $finding){$finding=sanitize_text_field((string)$finding);if(''!==$finding)$findings[]=$finding;}
        }
