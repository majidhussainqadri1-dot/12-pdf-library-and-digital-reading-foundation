...
final class PLDR_Future_Annotations {
    ...
    public static function import(int $edition_id, array $page): array {
        ...
        foreach($items as $annotation){
            ...
            foreach(array_slice($selectors,0,8) as $selector){if(!is_array($selector))continue;$type=sanitize_text_field((string)($selector['type']??''));if('FragmentSelector'===$type&&preg_match('/(?:^|[?&#;])?page=(\d+)/',(string)($selector['value']??''),$m))$page_no=absint($m[1]);if(in_array($type,array('TextQuoteSelector','CssSelector'),true)){$clean=self::selector($selector);$encoded=wp_json_encode($clean,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);if(is_string($encoded)&&strlen($encoded)<=640){$anchor=$encoded;if('TextQuoteSelector'===$type)$quote=(string)($clean['exact']??'');}}}
