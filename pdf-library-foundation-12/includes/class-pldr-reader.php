...
final class PLDR_Reader {
    ...
    private static function cover_token(int $edition_id): string { global $wpdb; $oid=(int)$wpdb->get_var($wpdb->prepare('SELECT object_id FROM '.PLDR_Core::table('derivatives').' WHERE edition_id=%d AND derivative_type=%s AND status=%s LIMIT 1',$edition_id,'cover','available')); if(!$oid)return''; $grant=PLDR_Access::issue_token($edition_id,$oid,'preview',get_current_user_id(),900); return is_array($grant)?$grant['url']:''; }
    private static function thumbnail_tokens(int $edition_id): array { global $wpdb; $rows=$wpdb->get_results($wpdb->prepare('SELECT page_number,object_id FROM '.PLDR_Core::table('derivatives').' WHERE edition_id=%d AND derivative_type=%s AND status=%s ORDER BY page_number ASC',$edition_id,'thumbnail','available'),ARRAY_A); $out=array(); foreach($rows as $r){$g=PLDR_Access::issue_token($edition_id,(int)$r['object_id'],'preview',get_current_user_id(),900);if(is_array($g))$out[(int)$r['page_number']]=$g['url'];} return $out; }
