...
final class PLDR_Crypto {
    ...
    public static function is_ready(?string &$message = null): bool {
        ...
        if (!$active || !isset($keys[$active])) {
            $message = 'PLDR_PDF_ACTIVE_KEY_ID is not present in the configured File 12 key ring.';
            return false;
        }
        $message = '';
        return true;
    ...
    public static function active_key_id(): string {
        ...
        if (defined('SPL_PDF_ACTIVE_KEY_ID')) {
            ...
            if ($legacy && isset($keys[$legacy])) {
                return $legacy;
            }
        }
        reset($keys);
        return $keys ? (string) key($keys) : '';
    }
    ...
    public static function encrypt_file(string $input, string $output, array &$metadata = array(), string &$error = ''): bool {
        ...
        try {
            ...
            if (!self::write_all($out, $header)) {
                throw new RuntimeException('Encrypted object header write failed.');
            }
            ...
            while (!feof($in)) {
                ...
                if (false === $plain) {
                    throw new RuntimeException('Source object read failed.');
                }
                if ('' === $plain) {
                    break;
                }
                ...
                if (false === $cipher || 16 !== strlen($tag)) {
                    throw new RuntimeException('Authenticated object encryption failed.');
                }
                if (!self::write_all($out, pack('N', strlen($cipher)) . $nonce . $tag . $cipher)) {
                    throw new RuntimeException('Encrypted object chunk write failed.');
                }
                $index++;
            }
            if (!fflush($out)) {
                throw new RuntimeException('Encrypted object flush failed.');
            }
            $metadata = array(
                'format' => self::MAGIC,
                'key_id' => $key_id,
    ...
    public static function verify_file(string $path, string $expected_sha256 = '', string &$error = ''): bool {
        ...
        $meta = array();
        $ok = self::stream_range($path, 0, null, static function (string $chunk) use ($ctx): void {
        ...
        $hash = hash_final($ctx);
        if ($expected_sha256 && !hash_equals(strtolower($expected_sha256), strtolower($hash))) {
            $error = 'Decrypted object checksum mismatch.';
            return false;
        }
        return true;
    }
    ...
    public static function decrypt_to_file(string $encrypted, string $output, string &$error = ''): bool {
        $out = fopen($output, 'wb');
        if (!$out) {
            $error = 'Temporary decrypt target could not be opened.';
            return false;
        }
        $meta = array();
        $ok = self::stream_range($encrypted, 0, null, static function (string $chunk) use ($out): void {
            if (!self::write_all($out, $chunk)) {
                throw new RuntimeException('Temporary decrypted object could not be written.');
            }
        }, $meta, $error);
        fclose($out);
        if (!$ok) {
    ...
    public static function stream_range(string $path, int $start, ?int $end, callable $writer, array &$metadata = array(), string &$error = ''): bool {
        ...
        if ($start < 0 || $start >= $size) {
            fclose($stream);
            $error = 'Requested byte range starts outside the object.';
            return false;
        }
        ...
        $first = intdiv($start, $chunk_size);
        $last = intdiv($end, $chunk_size);
        $record_size = 32 + $chunk_size;
        $offset = (int) $reader['header_length'] + ($first * $record_size);
        if (0 !== fseek($stream, $offset, SEEK_SET)) {
            fclose($stream);
            $error = 'Encrypted object range seek failed.';
            return false;
        }

        try {
            for ($index = $first; $index <= $last; $index++) {
                $length_raw = self::read_exact($stream, 4);
                if (false === $length_raw) {
                    throw new RuntimeException('Encrypted object chunk header is truncated.');
                }
                $unpacked = unpack('Nvalue', $length_raw);
                $length = (int) ($unpacked['value'] ?? 0);
                if ($length < 1 || $length > $chunk_size) {
                    throw new RuntimeException('Encrypted object chunk length is invalid.');
                }
                ...
                if (false === $nonce || false === $tag || false === $cipher) {
                    throw new RuntimeException('Encrypted object chunk is truncated.');
                }
                $aad = $reader['header_hash'] . pack('N', $index);
                $plain = openssl_decrypt($cipher, 'aes-256-gcm', $reader['key'], OPENSSL_RAW_DATA, $nonce, $tag, $aad);
                if (false === $plain) {
                    throw new RuntimeException('Encrypted object authentication failed.');
                }
                $chunk_start = $index * $chunk_size;
                $slice_start = max(0, $start - $chunk_start);
                $slice_end = min(strlen($plain) - 1, $end - $chunk_start);
                if ($slice_end >= $slice_start) {
                    $writer(substr($plain, $slice_start, $slice_end - $slice_start + 1));
                }
            }
        } catch (Throwable $e) {
            fclose($stream);
    ...
    private static function open_reader(string $path, string &$error = '') {
        if (!is_readable($path)) {
            $error = 'Encrypted private object is unavailable.';
            return false;
        }
        $stream = fopen($path, 'rb');
        if (!$stream) {
            $error = 'Encrypted private object could not be opened.';
            return false;
        }
        $magic = self::read_exact($stream, 4);
        if (!in_array($magic, array(self::MAGIC, self::LEGACY_MAGIC), true)) {
            fclose($stream);
            $error = 'Unsupported encrypted object format.';
            return false;
        }
        $version_raw = self::read_exact($stream, 1);
        $key_len_raw = self::read_exact($stream, 1);
        if (false === $version_raw || false === $key_len_raw) {
            fclose($stream);
            $error = 'Encrypted object header is incomplete.';
            return false;
        }
        $version = ord($version_raw);
        $key_len = ord($key_len_raw);
        if (self::VERSION !== $version || $key_len < 1 || $key_len > 64) {
            fclose($stream);
            $error = 'Encrypted object header is invalid.';
            return false;
        }
        $key_id = self::read_exact($stream, $key_len);
        ...
        $chunk_size = (int) ($chunk['value'] ?? 0);
        $size = self::unpack_u64($size_raw);
        if ($chunk_size < 65536 || $chunk_size > 8 * 1024 * 1024 || $size < 1) {
            fclose($stream);
            $error = 'Encrypted object limits are invalid.';
            return false;
        }
        $keys = self::keys();
        if (!isset($keys[$key_id])) {
            fclose($stream);
            $error = 'Required encryption key is not configured; restore the backed-up key ring.';
            return false;
        }
    ...
    private static function read_exact($stream, int $length) {
        $buffer = '';
        while (strlen($buffer) < $length && !feof($stream)) {
            $part = fread($stream, $length - strlen($buffer));
            if (false === $part) return false;
            $buffer .= $part;
        }
    ...
    private static function pack_u64(int $value): string {
        $high = intdiv($value, 4294967296);
        $low = $value - ($high * 4294967296);
        return pack('N2', $high, $low);
    }

    private static function unpack_u64(string $raw): int {
        $parts = unpack('Nhigh/Nlow', $raw);
        return $parts ? (int) (($parts['high'] * 4294967296) + $parts['low']) : 0;
    }
}
