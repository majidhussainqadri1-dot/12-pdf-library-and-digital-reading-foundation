...
final class PLDR_Future_A11y {
    ...
    public static function verify(int $edition_id,string $note='') {
        ...
        $updated=$wpdb->update(PLDR_Core::table('a11y_audits'),array('verified_by'=>get_current_user_id(),'verified_at'=>$verified_at,'updated_at'=>$verified_at),array('edition_id'=>$edition_id));
        if(1!==$updated)return PLDR_Core::machine_error('pldr_a11y_verify_store','Accessibility verification could not be persisted.',500);
        PLDR_Core::audit('edition',$edition_id,'accessibility_verified',array('document_id'=>$document_id,'note'=>$note));
