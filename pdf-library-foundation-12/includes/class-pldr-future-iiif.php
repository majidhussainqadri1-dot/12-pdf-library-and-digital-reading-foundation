...
final class PLDR_Future_IIIF {
    ...
    public static function manifest(string $public_id) {
        global $wpdb;
        $wpdb->last_error='';$doc=PLDR_Core::document_by_public_id($public_id);if(''!==(string)$wpdb->last_error)return PLDR_Core::machine_error('pldr_iiif_document_read','IIIF document state could not be read reliably.',503,array('degraded'=>true));if(!$doc)return PLDR_Core::machine_error('pldr_iiif_missing','Document not found.',404);
