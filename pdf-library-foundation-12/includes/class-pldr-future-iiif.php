...
final class PLDR_Future_IIIF {
    ...
    public static function manifest(string $public_id) {
        global $wpdb;
        $wpdb->last_error='';$doc=PLDR_Core::document_by_public_id($public_id);if(''!==(string)$wpdb->last_error)return PLDR_Core::machine_error('pldr_iiif_document_read','IIIF document state could not be read reliably.',503,array('degraded'=>true));if(!$doc)return PLDR_Core::machine_error('pldr_future_iiif_unavailable','IIIF manifest is unavailable for this document.',404);
        $wpdb->last_error='';$edition=PLDR_Core::current_edition((int)$doc['id']);if(''!==(string)$wpdb->last_error)return PLDR_Core::machine_error('pldr_iiif_edition_read','IIIF edition state could not be read reliably.',503,array('degraded'=>true));if(!$edition)return PLDR_Core::machine_error('pldr_future_iiif_unavailable','IIIF manifest is unavailable for this document.',404);
        $wpdb->last_error='';$allowed=PLDR_Access::can_access_edition((int)$edition['id'],'read',get_current_user_id());if(''!==(string)$wpdb->last_error)return PLDR_Core::machine_error('pldr_iiif_access_read','IIIF authorization state could not be verified reliably; no delivery grants were issued.',503,array('degraded'=>true));if(!$allowed)return PLDR_Core::machine_error('pldr_future_iiif_unavailable','IIIF manifest is unavailable for this document.',404);
        try {
