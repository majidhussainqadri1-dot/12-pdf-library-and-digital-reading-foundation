...
final class PLDR_Future_Citations {
    public static function export(int $edition_id, string $format, int $page = 0): array {
        ...
        $key = 'pldr-' . substr(str_replace('-', '', (string) $edition['public_id']), 0, 12);
        $csl = array('id' => $key, 'type' => 'book', 'title' => $title, 'author' => array(array('literal' => $author)), 'issued' => array('date-parts' => array(array($year ?: null))), 'publisher' => (string) $edition['publisher'], 'ISBN' => (string) $edition['isbn'], 'URL' => $url, 'edition' => (string) $edition['edition_label'], 'page' => $page ?: null);
        if ('csl-json' === $format) return array('format' => 'csl-json', 'content' => $csl);
        ...
        if (in_array($format, array('apa','mla','sabri'), true)) return array('format' => $format, 'content' => PLDR_Reader::citation($edition, $page, $format));
        if ('chicago' === $format) return array('format' => 'chicago', 'content' => trim($author . '. ' . $title . '. ' . ($edition['edition_label'] ? $edition['edition_label'] . '. ' : '') . ($edition['publisher'] ? $edition['publisher'] . ', ' : '') . ($year ?: 'n.d.') . '. ' . $url . ($page ? ', ' . $page : '') . '.'));
