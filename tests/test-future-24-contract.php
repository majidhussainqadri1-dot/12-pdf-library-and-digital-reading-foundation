...
$must = array(
    ...
    'offline-authorization','pldrOfflineReconnectReauth',
);
$combined = $entry . "\n" . $future . "\n" . $rest . "\n" . $js . "\n" . $css;
foreach ($must as $needle) if (strpos($combined, $needle) === false) { fwrite(STDERR, "Missing Future-24 contract marker: {$needle}\n"); exit(1); }
...
$allPhp = '';
foreach (new RecursiveIteratorIterator(new RecursiveDirectoryIterator($root . '/includes')) as $f) if ($f->isFile() && strtolower($f->getExtension()) === 'php') $allPhp .= "\n" . file_get_contents($f->getPathname());
foreach ($boundaryMarkers as $needle => $why) if (stripos($allPhp . "\n" . $js, $needle) === false) { fwrite(STDERR, "Missing boundary: {$why}\n"); exit(1); }
$forbidden = array(
...
foreach ($forbidden as $needle) if (strpos($combined . "\n" . $allPhp, $needle) !== false) { fwrite(STDERR, "Forbidden Future-24 regression marker: {$needle}\n"); exit(1); }
if (strpos($future, "future-reader-scholar.js") === false || strpos($future, "future-reader-personal.js") === false || strpos($future, "future-reader-vault.js") === false) { fwrite(STDERR, "Future-24 secondary clients are not enqueued by the canonical loader.\n"); exit(1); }
